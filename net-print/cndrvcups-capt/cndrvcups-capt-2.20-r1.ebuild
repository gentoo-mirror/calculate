...
SRC_URI="http://files.canon-europe.com/files/soft40567/software/CAPT_Printer_Driver_for_Linux_V${PV/./}_uk_EN.tar.gz"
...
DEPEND="=net-print/cndrvcups-common-${PVR}[cngplp=]
...
src_unpack() {
	unpack ${A}
	SRC="CAPT_Printer_Driver_for_Linux_V${PV/./}_uk_EN/Src/${PN}-${PVR/r/}.tar.gz"
	einfo "Unpacking ${SRC}"
	unpack "./${SRC}"
	einfo "Removing unnecessary sources"
	rm -rf "CAPT_Printer_Driver_for_Linux_V${PV/./}_uk_EN"
	cd ${S}
...
src_configure() {
	...
	cd ..
	for i in ${CNGPLPDIR} ${CNGPLPFDIR} backend pstocapt{,2,3} ppd ${STATUSDIR}; do
		cd ${i}
		NOCONFIGURE=1 ./autogen.sh
		econf
		cd ..
...
src_compile() {
	for i in ${CNGPLPDIR} ${CNGPLPFDIR} driver backend pstocapt{,2,3} ppd ${STATUSDIR}; do
		cd ${i}
		emake
		cd ..
	done
...
src_install() {
	for i in driver backend pstocapt{,2,3} ${STATUSDIR} ppd ${CNGPLPDIR} ; do
		cd ${i}
	...
	for i in ${STATUSDIR} driver backend pstocapt{,2,3} ppd ${CNGPLPDIR}; do
		docinto ${i}
	...
	einfo "Deleting /usr/share/caepcm/CNLC10{A,B}.ICC because there is the same in net-print/cndrvcups-lb"
	rm -fv ${D}/usr/share/caepcm/CNLC10{A,B}.ICC
}
...
pkg_postinst() {
	...
	einfo "See manual for more information at "
	einfo "http://software.canon-europe.com/files/soft31118/readme/README-capt-1.8xE.pdf"
	einfo 
