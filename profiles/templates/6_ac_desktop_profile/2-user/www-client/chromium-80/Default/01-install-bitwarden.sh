# Calculate pkg(www-plugins/chrome-bitwarden)!=&&ini(user_credentials.walutwarden_login)!=&&ini(user_credentials.waultwarden_pw)!= run=/bin/bash
...
/usr/share/chrome-bitwarden/configure.py #-ur_home_path-#/.config/chromium $plugin_id "#-ini(user_credentials.waultwarden_login)-#" "#-ini(user_credentials.waultwarden_pw)-#" &>/dev/null <<EOF
{
    "userId": {
	"data": {
	    "ciphers": {
